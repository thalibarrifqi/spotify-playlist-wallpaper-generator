...
/**
 * A drawable image source that also exposes its intrinsic dimensions
 * (HTMLImageElement and ImageBitmap both satisfy this).
 */
export type ImageLike = CanvasImageSource & { width: number; height: number };

/**
 * A minimal canvas abstraction that works both on the main thread
...
/**
 * Environment-specific capabilities the drawing pipeline needs.
 * The main-thread implementation uses HTMLImageElement + document.fonts;
 * the worker implementation uses fetch + createImageBitmap and skips fonts.
 */
...
async function renderBase(
  ...
  const loaded = await Promise.all(
    padded.map(async (image, index) => {
      try {
        const element = await env.loadImage(image.url);
...
/**
 * Full wallpaper drawing pipeline. Renders the background + artwork to an
 * offscreen canvas, composites it with the effect filter, applies overlay
 * effects (vignette/noise), and draws the optional title.
 *
 * Environment-agnostic: `env` supplies image loading, canvas creation, and
 * (optionally) font loading so the same code runs on the main thread and in
 * a Web Worker.
 */
export async function drawWallpaperCore(
  canvas: CanvasLike,
  images: AlbumImage[],
  config: WallpaperConfig,
  env: RenderEnvironment
  ...
  const base = env.createCanvas(config.width, config.height);
  const baseCtx = base.getContext("2d");
  if (!baseCtx) {
    throw new Error("Canvas 2D context is not supported in this browser");
  }
  await renderBase(baseCtx, images, config, env);
  ...
  ctx.drawImage(base as unknown as CanvasImageSource, drawX, drawY, drawWidth, drawHeight);
  ctx.filter = "none";
  ctx.restore();
  ...
  if (config.title) {
    ...
    if (env.ensureFontLoaded) {
      await env.ensureFontLoaded(font, textStyle.fontWeight, fontSize, config.title);
    }
